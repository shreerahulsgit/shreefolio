...
const DeskLamp = ({ position, isOn, onToggle }) => {
    ...
    useFrame((state, delta) => {
        const targetIntensity = isOn ? 12 : 0.8; // Boosted intensity for light
        const targetEmissive = isOn ? 8 : 0.4;  // Boosted emissive for bulb
        const targetHalo = isOn ? 0.8 : 0.08;   // Boosted opacity for halo
        if (lightRef.current) {
    ...
    return (
        <group position={position} onClick={(e) => { e.stopPropagation(); onToggle(); }}>
            {/* Heavy circular base — polished chrome */}
            <mesh position={[0, 0, 0]}>
            ...
            <group position={[0, 1.6, 0.6]}>
                ...
                <mesh rotation={[0.5, 0, 0]} scale={[0.95, 0.95, 0.95]}>
                    ...
                    <meshStandardMaterial 
                        ...
                        roughness={0.6}
                        emissive="#ffcc80"
                        emissiveIntensity={isOn ? 1.5 : 0.05} // Increased inner glow
                        side={THREE.FrontSide}
                    />
                </mesh>
                {/* Gold trim ring on shade */}
            ...
            <mesh position={[0, 1.45, 0.55]}>
                ...
                <meshStandardMaterial 
                    ...
                    color="#fff8e0" 
                    emissive="#ffcc44"
                    emissiveIntensity={0.4} // Base intensity (animation overrides this)
                />
            </mesh>
        ...
        <Float speed={1.5} rotationIntensity={0.02} floatIntensity={0.15}>
            <group>
                ...
                <group position={[0, -1.2, 0]}>
                    ...
                    {/* Lamp always visible */}
                    {/* Interactive Desk Lamp (top right) */}
                    <DeskLamp 
                    ...
                    {/* Floating "click the lamp" text — only when lamp is OFF */}
                    {!lampOn && (
                    ...
                    {/* Everything below only visible when lamp is ON */}
                    {lampOn && (<>

                    {/* --- Vertical Glass Sides --- */}
                    {[-3.8, 3.8].map((x, i) => (
                        <mesh key={`side-${i}`} position={[x, 2, 0]}>
                            ...
                            <meshPhysicalMaterial 
                                ...
                                thickness={0.5}
                                emissive="#6a3d99"
                                emissiveIntensity={lampOn ? 0.5 : 0} // Make glass glow slightly
                            />
                        </mesh>
                    ))}

                    {/* --- Horizontal Glass Shelves --- */}
                    {[4.8, 1.2, -2.2].map((y, i) => (
                        <group key={`shelf-${i}`}>
                            <mesh position={[0, y, 0]}>
                                ...
                                <meshPhysicalMaterial 
                                    ...
                                    thickness={0.3}
                                    emissive="#4a2080"
                                    emissiveIntensity={lampOn ? 0.6 : 0} // Make shelves glow
                                />
                            </mesh>

                            {/* Glow edges on each shelf — front */}
                            <GlowEdge 
                                position={[0, y, 0.9]} 
                                args={[7.5, 0.04, 0.02]} 
                                dimmed={dimmed}
                    ...
                    {/* Trophy/Award (top center-left) — 1.8x bigger */}
                    ...
                    {/* ═══ BOTTOM SHELF DECOR ═══ */}
                    {/* Crystal Gem (bottom left) — 1.8x bigger */}
                    ...
                    {/* Plant Pot (bottom center) — 1.6x bigger */}
                    ...
                    {/* Stacked books (bottom right) — 1.5x bigger */}
