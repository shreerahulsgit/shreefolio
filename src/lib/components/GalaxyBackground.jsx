...
const GalaxySpiral = ({ count = 4000, variant }) => {
    ...
    const { positions, colors, scales } = useMemo(() => {
        ...
        // Galaxy Parameters
        const branches = 3;
        const radius = 8;        // Slightly larger spread
        const spin = 0.8; 
        const randomness = 0.5;
        const randomnessPower = 3;
        
        // Colors
        // Inside: Bright White with slight violet tint
        ...
        for (let i = 0; i < count; i++) {
            ...
            positions[i3 + 1] = randomY; // Y is up/down in default, but we might rotate the group
            ...
            // Scale (randomize size for depth)
    ...
    const uniforms = useMemo(() => ({
        uTime: { value: 0 },
        uSize: { value: 200 * window.devicePixelRatio } // Adjust size base
    }), []);

    // Animation
    const targetRotationX = variant === 'tilted' ? Math.PI / 2.5 : 0.2; // Tilted vs subtle top/angle

    useFrame((state, delta) => {
        uniforms.uTime.value = state.clock.elapsedTime;
        
        // Very slow rotation
        pointsRef.current.rotation.y += delta * 0.05; // Rotate around center

        // Smooth transition for variant tilt
...
// Scene Wrapper to add Fog and interactions
const Scene = ({ variant }) => {
    ...
    React.useEffect(() => {
        ...
        scene.fog = new THREE.FogExp2('#050505', 0.08); // Very dark fog
    }, [scene]);
...
const GalaxyBackground = ({ variant = 'default' }) => {
    return (
        <div className="fixed inset-0 z-0 w-full h-full bg-[#050505] overflow-hidden">
            {/* HTML Gradient Overlay for vignette/text-protection */}
            <div 
            ...
            <Canvas
                camera={{ position: [0, 5, 8], fov: 45 }} // Higher up, looking down-ish
                gl={{ 
                    antialias: false,
