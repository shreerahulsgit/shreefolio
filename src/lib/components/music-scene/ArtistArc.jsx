...
const ArtistArc = ({ onSelectArtist, selectedArtist }) => {
    ...
    const radius = 10;
    const totalAngle = Math.PI / 1.5; // Spread over 120 degrees
    const startAngle = -totalAngle / 2;

    return (
        <group position={[0, 3.5, -5]}> {/* Raised up even higher as requested */}
            {artists.map((artist, i) => {
                const angle = startAngle + (i / (artists.length - 1)) * totalAngle;
                const x = Math.sin(angle) * radius;
                const z = (Math.cos(angle) * radius) - radius; // Curve
                // Look at center
                const rotY = -angle;
